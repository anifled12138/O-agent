...
Axiom 不是以聊天、Tool Calling 或 Plugin 数量为核心的 Agent 产品。它的
...
> Axiom 是一个能够针对现实任务自主设计、实验并晋升下一代求解系统的本地
> 递归自举 Agent。
...
Axiom 不与通用 Plugin Runtime 比“能否表达某个功能”。任何足够通用的插件
系统都能承载任意可计算功能。Axiom 竞争的是以下结果：

> 在相同基础模型、相近预算和相同任务条件下，Axiom 能否比固定 Harness
...
用户始终与一个连续的 Axiom 身份交互，但该身份背后的求解系统可以产生多个
...
- Axiom 可以承认当前能力不足，而不是无限重试；
- Axiom 可以为能力缺口创建可运行的实验；
- Axiom 可以生成整个求解架构，而不只生成 Tool；
- Axiom 可以用同条件基线证明候选是否更好；
- Axiom 可以让通过评测的后继系统继续参与下一轮自举；
...
Axiom 从七个维度描述 Agent 能力，而不是只维护 Tool Catalog：
...
用户直接描述目标。Axiom 优先使用当前 Stable Generation 完成任务，不展示
...
属于 Axiom Identity；Generation 只是其当前求解系统。用户始终面对一个产品
主体，而不是被迫管理大量机器人实例。
...
环境反馈和系统组合层。Axiom 不应把系统放大误称为基础模型参数能力突破。
...
这些问题不阻塞 M0/M1。首要事实是：Axiom 的核心对象从 Plugin 转为完整
Agent Generation，核心闭环从“生成并安装”转为“提出、实验、选择、晋升并
递归产生后继”。

## 28. 研究基线

本 PRD 将以下现有方向视为设计基线，而不是 Axiom 独有能力：
...
Axiom 的产品假设不是上述任一构件首次出现，而是把真实用户目标、完整 Agent
