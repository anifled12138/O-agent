...
func writeProjectV2(p Project, manifest pluginmanifest.Manifest, shape string) error {
	...
	files[filepath.Join(p.SourceDir, ".gitignore")] = []byte("build/\n")
...
func commitGeneratedProject(ctx context.Context, dir string) error {
	if _, err := os.Stat(filepath.Join(dir, ".git")); os.IsNotExist(err) {
		if _, err = gitCommand(ctx, dir, "", "init", "-b", "main"); err != nil {
			return err
		}
		if _, err = gitCommand(ctx, dir, "", "config", "user.name", "O Plugin Forge"); err != nil {
			return err
		}
		if _, err = gitCommand(ctx, dir, "", "config", "user.email", "forge@o.local"); err != nil {
			return err
		}
	}
	...
	_, err := gitCommand(ctx, dir, "", "commit", "-m", "feat: generate initial plugin")
	return err
}
func commitSourceChange(ctx context.Context, dir, path string) error {
	if _, err := gitCommand(ctx, dir, "", "add", "--", path); err != nil {
		return err
	}
	_, err := gitCommand(ctx, dir, "", "commit", "-m", "feat: update "+path)
	return err
}
