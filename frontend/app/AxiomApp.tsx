...
export default function AxiomApp() {
  ...
  return <main className="app-shell">
    ...
    {settingsOpen && <Settings providers={providers} onClose={() => setSettingsOpen(false)} onCreated={(next) => { setProviders((items) => [next, ...items]); setNotice('Provider saved'); }} />}
  </main>;
}
...
function Settings({ providers, onClose, onCreated }: { providers: Provider[]; onClose: () => void; onCreated: (provider: Provider) => void }) {
  const [busy, setBusy] = useState(false); const [status, setStatus] = useState(''); const defaultName = useMemo(() => providers.length ? `Provider ${providers.length + 1}` : 'Primary model', [providers.length]);
  async function save(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setBusy(true); setStatus(''); const form = new FormData(event.currentTarget); try { const created = await request<Provider>('/providers', { method: 'POST', body: JSON.stringify(Object.fromEntries(form)) }); onCreated(created); setStatus('Provider mounted successfully.'); (event.target as HTMLFormElement).reset(); } catch (error) { setStatus(error instanceof Error ? error.message : 'Could not save provider'); } finally { setBusy(false); } }
  ...
  return <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="settings-modal"><header><div><span className="eyebrow">SYSTEM / PROVIDERS</span><h2>Model connections</h2></div><button onClick={onClose}>×</button></header><p className="modal-lead">Connect an API model. Credentials are encrypted at rest by the local Go service and are never returned to the browser.</p>{!!providers.length && <div className="provider-list">{providers.map((item) => <div key={item.id}><span className="model-icon">M</span><div><b>{item.name}</b><small>{item.model} · {item.baseUrl}</small></div><button onClick={() => test(item.id)}>Test</button></div>)}</div>}<form onSubmit={save} className="provider-form"><p>ADD OPENAI-COMPATIBLE PROVIDER</p><div className="form-grid"><label>CONNECTION NAME<input name="name" defaultValue={defaultName} required /></label><label>MODEL ID<input name="model" placeholder="gpt-5 / deepseek-chat" required /></label><label className="wide">BASE URL<input name="baseUrl" defaultValue="https://api.openai.com/v1" required /></label><label className="wide">API KEY<input name="apiKey" type="password" placeholder="sk-…" required /></label></div><input type="hidden" name="kind" value="openai-compatible"/>{status && <div className="form-status">{status}</div>}<button className="primary-button" disabled={busy}>{busy ? 'Encrypting…' : 'Save connection'}<span>→</span></button></form></section></div>;
}
